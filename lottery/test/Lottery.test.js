...
describe("Lottery Contract", () => {
  ...
  it("allows one account to ether", async () => {
  ...
  it("requires a minimum amount of ether to enter", async () => {
    try {
      ...
      assert(false); // try 내부 구문에 오류가 없으면 여기로 넘어오고, test 는 fail
    } catch (err) {
      assert(err); // truthness 체크 / assert.ok 는 existence 를 체크
    }
  });
  ...
  it("final version", async () => {
    ...
    assert(difference > web3.utils.toWei("2.0"));
  });
});
